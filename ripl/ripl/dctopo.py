...
class JellyfishTopo(StructuredTopo):
    ...
    class JellyfishNodeID(NodeID):
        ...
        def __init__(self, sw=0, host=0, name = None, dpid = None):
    ...
    def __init__(self, num_switches, num_switch_ports, adjlist_file, speed=1.0):
        '''Init.
        ...
        '''
        switch = StructuredNodeSpec(num_switch_ports - 1, 1, speed, speed, 'switch')
        host = StructuredNodeSpec(1, 0, speed, speed, 'host')
        super(JellyfishTopo, self).__init__([switch, host], [StructuredEdgeSpec(speed)])

        self.id_gen = JellyfishTopo.JellyfishNodeID

        # Create switches and hosts (one host per switch)
        switches = []
        hosts = []
        for i in range(0, num_switches):
            switch_id = self.id_gen(i, 1).name_str()
            switch_opts = self.def_nopts(self.LAYER_EDGE, switch_id)
            switches.append(self.addSwitch(switch_id, **switch_opts))
            
            host_id = self.id_gen(i, 2).name_str()
        ...
        with open(adjlist_file) as f:
            for line in f:
                ...
                for dest_node in tokens[1:]:
                    self.addLink(switches[int(source_node)], switches[int(dest_node)], bw=10)
